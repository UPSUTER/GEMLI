visualize_as_network <- function(GEMLI_items, cutoff=70, max_edge_with=5, display_orphan=F, include_labels=F, ground_truth=F, highlight_FPs=T, layout_style='fr', cell_type_colors=T)
{
  ...
  vertex_color[is.na(vertex_color)] = "white"
    ...
    edge_color = igraph::edge_attr(network_graph, "weight") < 0; edge_color[edge_color==T] <- 'red'; edge_color[edge_color==F] <- 'grey'
      ...
      if (layout_style=='grid') {plot.igraph(network_graph, vertex.size=3, vertex.label.cex=0.5, vertex.color=vertex_color, vertex.label.color="black", vertex.label.family="Arial", edge.color=edge_color, layout=layout.grid(network_graph), rescale=TRUE, edge.width=edge_width, vertex.label.dist=0.5, vertex.label.degree=pi*1.5)}
      legend("bottomright", legend = GEMLI_items[['cell_type_color']]$cell.type, pch = 16, col = GEMLI_items[['cell_type_color']]$color, bty = "n", horiz=F, xpd=TRUE, inset=c(0, -.10),ncol=4)
  }
