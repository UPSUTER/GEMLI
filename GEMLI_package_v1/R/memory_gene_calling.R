memory_gene_calling <- function(GEMLI_items, valid_lineage_sizes=2:5, use_median=T, ground_truth=F, cell_fate)
{
    if (class(GEMLI_items)=='list') {
        ...
        predicted_lineages = GEMLI_items[['predicted_lineages']]
        predicted_lineage_table = GEMLI_items[['predicted_lineage_table']]
        cell_fate_analysis = GEMLI_items[['cell_fate_analysis']]
    ...
    if (ground_truth) {lineage_dict = barcodes} else {
        if (length(predicted_lineages)>0){lineage_dict = predicted_lineages} else {
            lineage_dict = predicted_lineage_table$clone.ID; names(lineage_dict) = predicted_lineage_table$cell.ID}}
    if (hasArg(cell_fate)){match<-cell_fate_analysis[cell_fate_analysis$cell.fate ==cell_fate,]
